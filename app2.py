...
######################
#------------------------------------------------------------------------------------------------
#식약처 의약품안전사용서비스(DUR) 품목 정보 api 주소
...
#response에 item 정보가 없으면 "병용가능" 문자열 반환
#response에 item 정보가 있다면 {의약품명:증상} 형태의 dictionary로 저장후 
#사용자가 입력한 두 의약품이 병용금기인지 확인 후 병용금기라면 병용금기 사실과 증상을 문자열로 반환
#병용금기가 아니라면 "병용가능" 문자열로 반환
...
def kid_resp(endpoint, params):
    ...
    if response.status_code == 200:
        ...
        return "응답이 취소되었습니다. 오류코드:" + str(response.status_code)
#----------------------------------------------------------------------------------------#
#질문 유형과 api 호출 주소(endpoint)를 맵핑해서 DUR 정보를 문자열 형태로 반환
